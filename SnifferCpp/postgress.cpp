...
int InsertRecord(PGconn *conn, char* table_name, char* call_date, char* call_id, char* result)
{
	// Append the SQL statement
	char sSql[256];
	sprintf(sSql, "INSERT INTO %s VALUES('%s', '%s', '%s')", table_name, call_date, call_id, result);
...
{
	PGconn *conn = NULL;
	conn = ConnectDatabase("testdb", "mingri", "", "127.0.0.1", 5432);
	//conn = MakeDatabase("testdb", "Alexander-PC", "", "127.0.0.1", "5432");
	CreateTable(conn, "VadTable");
	InsertRecord(conn, "VadTable", "2016-08-31", "1", "OK");
	InsertRecord(conn, "VadTable", "2016-08-31", "2", "OK");
	GetAllRecords(conn, "VadTable");
	...
	while (1)
	{
		...
		if (c == 'd')
		{
			RemoveAllRecords(conn, "VadTable");
			DropTable(conn, "VadTable");
			break;
		}
		else if (c == 'q')
			break;
...
PGconn* postgress_connect()
{
    ...
    //printf("username=%s\n",getlogin());
    sprintf(usrname,"mingri");
	sprintf(conninfo_str, "user='%s' password='' dbname='vad_db' hostaddr='127.0.0.1' port=5432", usrname);
	conn = PQconnectdb(conninfo_str);
	...
	PGresult *res = NULL;
	sprintf(sSql, "CREATE TABLE IF NOT EXISTS vad_result (calltime char(30), callid char(64), vadres int)");
	res = PQexec(conn, sSql);
